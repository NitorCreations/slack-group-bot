...
public class UsergroupHandler {
	...
	public Response handleUsergroupCommand(SlashCommandRequest req, SlashCommandContext ctx) {
		...
		try {
			...
			if (finalizeUsergroupCommand(userId, command, usergroupName, responseChannel)) {
				logger.error("The operation to {} the group {} " + "has been successful", command, usergroupName);
				return ctx.ack();
			} else {
				logger.error("The operation to {} the group {} " + "has failed", command, usergroupName);
				return ctx
						.ack(String.format("The operation to %s the group %s has failed :x:", command, usergroupName));
			}
		} catch (Exception e) {
			logger.info("Invalid parameters: command/group is " + "missing from the input");
			return ctx
					.ack("The operation has failed: please check " + "that you have written the command correctly :x:");
		}
	}
	...
	private boolean finalizeUsergroupCommand(String userId, String command, String usergroupName,
		...
		if (!similarNames.isEmpty() && command.equalsIgnoreCase("join")) {
			...
			blockMessager.similarGroupsMessage(usergroupName, similarNames, responseChannel, userId);
			return true;
		}
		...
		if (usergroup == null) {
			messageUtil
					.sendEphemeralResponse(
							String.format("Due to an unexpected I/O or Slack API error, the "
									+ "group %s was not found or created :warning:", usergroupName),
							userId, responseChannel);
			return false;
		}
	...
	public boolean addUserToGroup(String userId, Usergroup group, String responseChannel) {
		if (!usergroupUtil.checkIfAvailable(group)) {
			messageUtil.sendEphemeralResponse(
					String.format("Due to an unexpected I/O or Slack API error, the "
							+ "group %s was found but not enabled :warning:", group.getName()),
					userId, responseChannel);
			return false;
		}
		...
		if (usergroupUtil.userInGroup(userId, users)) {
			...
			messageUtil.sendEphemeralResponse(String.format("You are already in the group %s. "
					+ "You can only join groups " + " you are not a part of :warning:", group.getName()), userId,
					responseChannel);
			return false;
			...
			if (!success) {
				...
				messageUtil.sendEphemeralResponse(
						String.format("You have been added to the group %s :white_check_mark:", group.getName()),
						userId, responseChannel);
			}
			return success;
		}
	...
	public boolean removeUserFromGroup(String userId, Usergroup group, String responseChannel) {
		...
		if (!usergroupUtil.userInGroup(userId, users) || group.getDateDelete() != 0) {
			...
			messageUtil.sendEphemeralResponse(String.format(
					"You are not in the group %s. " + "You can only leave groups " + "you are a part of :warning:",
					group.getName()), userId, responseChannel);
			return false;
		}

		logger.info("Checking if the user is " + "the last member of the group {}", group.getName());
		List<String> modifiedUsers = users.stream().filter(u -> !u.equals(userId)).collect(Collectors.toList());
		...
		if (result) {
			messageUtil.sendEphemeralResponse(
					String.format("You have been removed from the group %s :white_check_mark:", group.getName()),
					userId, responseChannel);
		} else {
			messageUtil.sendEphemeralResponse(
					String.format("Due to an unexpected I/O or Slack API error, "
