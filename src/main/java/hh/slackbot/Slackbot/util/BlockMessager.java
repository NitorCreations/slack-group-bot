...
public class BlockMessager {
  ...
  private List<LayoutBlock> similarGroupsLayout(String actual, List<String> similar) {
    ...
    layout.add(
        section(section -> section.text(markdownText(String.format("Given group was *%s*", actual)))
            .blockId("header")));

    layout.add(actions(actions -> actions.elements(Arrays.asList(button(b -> b
        .text(plainText(pt -> pt.text("create and join"))).value(actual).actionId("btn_create"))))
        .blockId("creation")));
