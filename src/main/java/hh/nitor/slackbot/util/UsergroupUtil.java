...
public class UsergroupUtil {
	...
	public List<Usergroup> getUserGroups() {
		...
		try {
			UsergroupsListResponse resp = client.usergroupsList(
					UsergroupsListRequest.builder().token(TOKEN).includeDisabled(true).includeUsers(true).build());
			...
			logger.error(
					String.format("Failed to retrieve all groups due " + "to Slack API Error: %n %s", e.getMessage()));
		}
	...
	public List<String> getUsergroupUsers(String groupId) {
		...
		try {
			UsergroupsUsersListResponse resp = client
					.usergroupsUsersList(UsergroupsUsersListRequest.builder().token(TOKEN).usergroup(groupId).build());
			...
			logger.error(
					String.format("Failed to retrieve the group's users due to " + "IO Error: %n %s", e.getMessage()));
		} catch (SlackApiException e) {
			logger.error(String.format("Failed to retrieve the group's users due to " + "Slack API Error: %n %s",
					e.getMessage()));
		}
	...
	public Usergroup createUsergroup(String name) {
		...
		try {
			UsergroupsCreateResponse resp = client
					.usergroupsCreate(UsergroupsCreateRequest.builder().token(TOKEN).name(name).handle(name).build());
	...
	public boolean enableUsergroup(String id) {
		try {
			...
			logger.error(
					String.format("Failed to enable the group due to " + "Slack API Error: %n %s", e.getMessage()));
		}
	...
	public boolean disableUsergroup(String id) {
		...
		try {
			...
			logger.error(
					String.format("Failed to disable the group " + "due to Slack API Error: %n %s", e.getMessage()));
		}
	...
	public boolean updateUsergroupUserlist(List<String> users, String groupId) {
		...
		try {
			client.usergroupsUsersUpdate(UsergroupsUsersUpdateRequest.builder().token(System.getenv("SLACK_BOT_TOKEN"))
					.usergroup(groupId).users(users).build());
			logger.info("The group's user list successfully updated");
			return true;
		} catch (IOException e) {
			logger.error(String.format("Failed to update the group's user list " + "due to IO Error: %n %s",
					e.getMessage()));
		} catch (SlackApiException e) {
			logger.error(String.format("Failed to update the group's user list " + "due to Slack API Error: %n %s",
					e.getMessage()));
		}
		return false;
	}
