...
class UsergroupHandlerTest {
	...
	private void initUserGroups() {
		...
		List<Usergroup> groups = Arrays.asList(
				Usergroup.builder().id("1111").users(users).name("sample group").dateDelete(0).build(),
				Usergroup.builder().id("2222").users(new ArrayList<String>()).name("empty group").dateDelete(0).build(),
				Usergroup.builder().id("3333").users(users2).name("single group").dateDelete(0).build(),
				Usergroup.builder().id("4444").users(users2).name("disabled group").dateDelete(1).build(),
				Usergroup.builder().id("5555").users(users).name("sample2 group").dateDelete(0).build());
	...
	void addUserBackToDisabledGroup() {
		...
		verify(groupUtil).updateUsergroupUserlist(new ArrayList<String>(Arrays.asList("user1")), "4444");
		verify(mockCtx).ack();
	}
	...
	void typoFails() {
		...
		verify(msgUtil).sendEphemeralResponse(anyList(), eq("Groups with similar names"), eq(userId), eq("channel_id"));
		verify(mockCtx).ack();
	}
